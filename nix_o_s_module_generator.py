...
def nix_value(value: Any) -> str:
    ...
    if value is None:
        ...
        if not value:
            return "{ }"
        items = " ".join(
            f"{sanitize_nix_identifier(k)} = {nix_value(v)};" for k, v in value.items()
        )
        return f"{{ {items} }}"
    else:
        return "null"
...
def pfm_type_to_nix_type(
    pfm_type: str, subkeys: list | None = None, range_list: list | None = None
) -> str:
...
def generate_option(subkey: dict, indent: int = 4, seen_names: set = None) -> str:
    ...
    if pfm_type == "dictionary" and subkeys:
        ...
        and subkeys
        and len(subkeys) > 0
        and subkeys[0].get("pfm_type") == "dictionary"
    ):
...
def generate_nix_module(plist_path: Path, category: str = "") -> str:
    ...
    lines.append(
        f'    description = "{escaped_title} ({domain}) payload instances, keyed by instance name. Use \\"default\\" if you only need one.";'
    )
    lines.append("  };")
    lines.append("}")
