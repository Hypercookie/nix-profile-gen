  ...
  payloadModule = {
    options = {
      ...
      _keyNames = lib.mkOption {
        ...
        default = [ "DirectoryURL" "ClientIdentifier" "KeySize" "KeyType" "HardwareBound" "Subject" "SubjectAltName" "UsageFlags" "ExtendedKeyUsage" "Attest" "KeyIsExtractable" "AllowAllAppsAccess" ];
...
in
{
  options.programs.macprofile.payloads."apple-com-apple-security-acme" = lib.mkOption {
    type = types.attrsOf (types.submodule payloadModule);
    default = { };
    description = "ACME Certificate (com.apple.security.acme) payload instances, keyed by instance name. Use \"default\" if you only need one.";
  };
}