  ...
  payloadModule = {
    options = {
      ...
      _keyNames = lib.mkOption {
        ...
        default = [ "ServerURL" "ServerURLPinningCertificateUUIDs" "ServerCapabilities" "Topic" "IdentityCertificateUUID" "ManagedAppleID" "AccessRights" "CheckInURL" "CheckInURLPinningCertificateUUIDs" "CheckOutWhenRemoved" "PinningRevocationCheckRequired" "PromptUserToAllowBootstrapTokenForAuthentication" "SignMessage" "UseDevelopmentAPNS" "AssignedManagedAppleID" "EnrollmentMode" "RequiredAppIDForMDM" ];
      ...
      PinningRevocationCheckRequired = lib.mkOption {
        type = types.nullOr (types.bool);
        default = null;
        description = "If 'true', the system fails the connection attempt unless it obtains a verified positive response during certificate revocation checks. If 'false', the system performs revocation checks on a best-attempt basis, where failure to reach the server isn't considered fatal.";
      };
...
in
{
  options.programs.macprofile.payloads."apple-com-apple-mdm" = lib.mkOption {
    type = types.attrsOf (types.submodule payloadModule);
    default = { };
    description = "MDM (com.apple.mdm) payload instances, keyed by instance name. Use \"default\" if you only need one.";
  };
}