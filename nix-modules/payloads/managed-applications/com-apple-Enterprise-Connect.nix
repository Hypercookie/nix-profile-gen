  ...
  payloadModule = {
    options = {
      ...
      _keyNames = lib.mkOption {
        ...
        default = [ "adRealm" "PFC_SegmentedControl_0" "launchAtLogin" "runPeriodicStateCheck" "orgUsernameLabel" "prepopulatedUsername" "preferredDC" "setupReminderNagInterval" "getRenewableTGT" "showMenuExtra" "checkShowLegacyCertificates" "showKeychainIdentities" "disableQuitMenu" "orgLogoPath" "debugMode" "syncLocalPassword" "pwReqComplexity" "pwReqComplexityDisableUnicode" "pwReqLength" "pwReqText" "pwReqHistoryCount" "pwReqMinimumPasswordAge" "disablePasswordFunctions" "disablePasswordExpirationChecking" "runPasswordChangeScriptOnLocalPasswordSync" "passwordChangeScriptPath" "passwordChangeURL" "passwordExpireOverride" "passwordNotificationDays" "checkForNetworkType" "checkForNetworkServer" "connectDelay" "connectionCompletedScriptPath" "connectReminderNagInterval" "dailyReconnectTime" "runAuditScript" "runAuditScriptPath" "mountNetworkHomeDirectory" "pfc_preventUserShares" "shares" "managedshares" "shareMountWaitSeconds" "smartCardMode" "showUsernameWithSmartcard" "destroyKerbTicketUponCardRemoval" ];
...
in
{
  options.programs.macprofile.payloads."managed-applications-com-apple-Enterprise-Connect" = lib.mkOption {
    type = types.attrsOf (types.submodule payloadModule);
    default = { };
    description = "Enterprise Connect (com.apple.Enterprise-Connect) payload instances, keyed by instance name. Use \"default\" if you only need one.";
  };
}