  ...
  payloadModule = {
    options = {
      ...
      _keyNames = lib.mkOption {
        ...
        default = [ "authURL" "apiURL" "logFilter" "accountSlug" "hideAdminPortalMenuItem" "hideResourceList" "connectOnStart" "startOnLogin" "disableUpdateCheck" "supportURL" ];
...
in
{
  options.programs.macprofile.payloads."managed-applications-dev-firezone-firezone" = lib.mkOption {
    type = types.attrsOf (types.submodule payloadModule);
    default = { };
    description = "Firezone (dev.firezone.firezone) payload instances, keyed by instance name. Use \"default\" if you only need one.";
  };
}