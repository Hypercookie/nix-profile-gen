  ...
  payloadModule = {
    options = {
      ...
      _keyNames = lib.mkOption {
        ...
        default = [ "ADDomain" "mapKerberosPrincipalName" "shouldUpdateKerberosUserPrincipalADDomain" "clientID" "clientSecret" "CreateAdminUser" "skipUserSetupBuddy" "allowUsersClaim" "allowedUsersArray" "allowLoginIfMemberOfGroup" "CreateAdminIfGroupMember" "claimsToAddToLocalUserAccount" "systemInfoAdditionsArray" "shouldSwitchToLoginWindowWhenLocked" "LocalFallback" "shouldActivateSystemInfoButton" "discoveryURL" "EnableFDE" "EnableFDERecoveryKey" "EnableFDERecoveryKeyPath" "EnableFDERekey" "loginWindowWidth" "loginWindowHeight" "loadPageTitle" "loadPageInfo" "shouldHideLoginWindowLogo" "loginWindowLogoPath" "loginWindowBackgroundImageURL" "loginWindowBackgroundImageAlpha" "loginWindowSecondaryMonitorsBackgroundImageURL" "menuItemIconData" "menuItemIconCheckedData" "menuItemWindowBackgroundImageURL" "menuItemWindowBackgroundImageAlpha" "shouldLoginWindowBackgroundImageFillScreen" "shouldLoginWindowSecondaryMonitorsBackgroundImageFillScreen" "loginWindowSecondaryMonitorsBackgroundAlpha" "shouldRemoveMenuItemAutoLaunch" "shouldUseADNativePasswordChangeMenuItem" "passwordChangeURL" "redirectURI" "shouldShowCloudLoginByDefault" "refreshRateHours" "refreshRateMinutes" "scopes" "shouldSetGoogleAccessTypeToOffline" "googleHostDomain" "shouldPromptForADPasswordChange" "shouldShowSignInMenuItem" "shouldSuppressLocalPasswordPrompt" "shouldPreferLocalLoginInsteadOfCloudLogin" "shouldVerifyPasswordWithRopg" "shouldUseBasicAuthWithROPG" "ropgClientID" "ropgClientSecret" "shouldUseROPGForLoginWindowLogin" "shouldUseROPGForMenuLogin" "shouldUseROPGForPasswordChangeChecking" "shouldUseLDAPForPasswordChangeChecking" "resource" "ropgResponseValue" "hideIfPathExists" "aliasName" "autoRefreshLoginTimer" "cloudLoginText" "shouldShowAboutMenu" "shouldSkipFileVaultLogin" "shouldSkipFileVaultLoginAdmin" "shouldShowMenuBarSignInWithoutLoginWindowSignin" "refreshBannerText" "shouldShowRefreshBanner" "shouldShowConfigureWifiButton" "shouldShowShutdownButton" "shouldShowRestartButton" "shouldShowSystemInfoButton" "shouldShowPreferencesOnStart" "shouldPromptForMigration" "shouldAllowKeyComboForMacLoginWindow" "keyCodeForLoginWindowChange" "shouldShowMacLoginButton" "shouldShowLocalOnlyCheckbox" "usernamePlaceholder" "passwordPlaceholder" "shouldShowSupportStatus" "shouldShowQuitMenu" "shouldShowVersionInfo" "showDebug" "username" "KeychainReset" "PasswordOverwriteSilent" "HideExpiration" "localAdminUserName" "localAdminPassword" "accountLockedPasswordDialogTitle" "shouldSkipSettingSecureTokenForAdmin" "shouldHideSecureTokenStatus" "accountLockedPasswordDialogText" "resetPasswordDialogTitle" "systemInfoButtonTitle" "verifyPassword" "shouldDetectNetworkToDetermineLoginWindow" "idpHostName" "idpHostNames" "AdditionalADDomains" "ccidSlotName" "shouldAllowLoginCardSetup" "adUserAttributesToAddToLocalUserAccount" "passwordElementID" "map_firstname" "map_lastname" "map_fullname" "map_username" "map_fullusername" "map_uid" "primaryGroupID" "map_password_expiry" "settingsOverrideScriptPath" "upnSuffixToDomainMappings" "menuItems" "Shares" "HomeMountEnabled" "HomeAppendDomain" "shareMenuItemName" "shouldAllowEmptyLocalPassword" ];
      ...
      googleHostDomain = lib.mkOption {
        type = types.nullOr (types.str);
        default = null;
        description = "When using Google IdP, set the HD key to show the user's domain";
      };
      ...
      shouldSkipSettingSecureTokenForAdmin = lib.mkOption {
        type = types.nullOr (types.bool);
        default = null;
        description = "Skip setting a secure token for the local admin user specified in XCreds. The token will be attempted if the local admin does not already have a secure token and a boot strap token is available.";
      };

      shouldHideSecureTokenStatus = lib.mkOption {
        type = types.nullOr (types.bool);
        default = null;
        description = "Hide the status of SecureToken users in the popover status view";
      };
      ...
      shouldAllowEmptyLocalPassword = lib.mkOption {
        type = types.nullOr (types.bool);
        default = null;
        description = "Allow local user with empty password to log in";
      };
...
in
{
  options.programs.macprofile.payloads."managed-applications-com-twocanoes-xcreds" = lib.mkOption {
    type = types.attrsOf (types.submodule payloadModule);
    default = { };
    description = "XCreds (com.twocanoes.xcreds) payload instances, keyed by instance name. Use \"default\" if you only need one.";
  };
}