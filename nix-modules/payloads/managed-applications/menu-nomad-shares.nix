  ...
  payloadModule = {
    options = {
      ...
      _keyNames = lib.mkOption {
        ...
        default = [ "Version" "PFC_SegmentedControl_0" "HomeMount" "Shares" "IgnoreShareNames" ];
...
in
{
  options.programs.macprofile.payloads."managed-applications-menu-nomad-shares" = lib.mkOption {
    type = types.attrsOf (types.submodule payloadModule);
    default = { };
    description = "NoMAD Shares (menu.nomad.shares) payload instances, keyed by instance name. Use \"default\" if you only need one.";
  };
}