  ...
  payloadModule = {
    options = {
      ...
      _keyNames = lib.mkOption {
        ...
        default = [ "RequiredArguments" "OptionalArguments" ];
      ...
      OptionalArguments = lib.mkOption {
        type = types.nullOr (types.submodule {
          options = {
            ...
            NotificationStyle = lib.mkOption {
              type = types.nullOr (types.enum [ "pseudo-alert" "pseudo-banner" "alert" "banner" ]);
              default = null;
              description = "Correlates with the --style option of Dialog. Defaults to pseudo-alert on Dialog 3.1 or newer, ignored on any older version.";
            };
            Title = lib.mkOption {
...
in
{
  options.programs.macprofile.payloads."managed-applications-com-secondsonconsulting-renew" = lib.mkOption {
    type = types.attrsOf (types.submodule payloadModule);
    default = { };
    description = "Renew - Second Son Consulting (com.secondsonconsulting.renew) payload instances, keyed by instance name. Use \"default\" if you only need one.";
  };
}