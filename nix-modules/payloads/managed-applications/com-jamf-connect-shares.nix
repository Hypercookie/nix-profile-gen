  ...
  payloadModule = {
    options = {
      ...
      _keyNames = lib.mkOption {
        ...
        default = [ "Version" "PFC_SegmentedControl_0" "HomeMount" "Shares" ];
...
in
{
  options.programs.macprofile.payloads."managed-applications-com-jamf-connect-shares" = lib.mkOption {
    type = types.attrsOf (types.submodule payloadModule);
    default = { };
    description = "Jamf Connect Shares (com.jamf.connect.shares) payload instances, keyed by instance name. Use \"default\" if you only need one.";
  };
}