  ...
  payloadModule = {
    options = {
      ...
      _keyNames = lib.mkOption {
        ...
        default = [ "PFC_SegmentedControl_0" "app.startAtLogin" "authentication.defaultDomain" "authentication.enforceDomain" "privacy.checkCompromisedWebsites" "privacy.checkMfa" "privacy.checkPasskeys" "security.blockSleepEnabled" "security.revealWifiQrCodes" "browsers.other_trusted_apps.enabled" "security.autofill.autosubmit" "security.authenticatedUnlock.requireAccountPasswordAfter" "security.authenticatedUnlock.appleTouchId" "security.authenticatedUnlock.appleWatchUnlock" "security.revealPasswords" "security.autolock.onDeviceLock" "security.autolock.onWindowClose" "security.autolock.minutes" "security.clipboard.clearAfter" "security.deviceClipboardSharing" "privacy.downloadRichIcons" "privacy.checkHibp" "updates.autoUpdate" "updates.updateChannel" ];
      ...
      "authentication.defaultDomain" = lib.mkOption {
        type = types.nullOr (types.str);
        default = null;
        description = "Set the default sign-in address for the 1Password app. Users will be prompted to sign in to this account by default.";
      };

      "authentication.enforceDomain" = lib.mkOption {
        type = types.nullOr (types.bool);
        default = null;
        description = "When enabled, users cannot change the sign-in address from the default value set in authentication.defaultDomain.";
      };
...
in
{
  options.programs.macprofile.payloads."managed-applications-com-1password-1password" = lib.mkOption {
    type = types.attrsOf (types.submodule payloadModule);
    default = { };
    description = "1Password 8 (com.1password.1password) payload instances, keyed by instance name. Use \"default\" if you only need one.";
  };
}