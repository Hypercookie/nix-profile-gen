  ...
  payloadModule = {
    options = {
      ...
      _keyNames = lib.mkOption {
        ...
        default = [ "OIDCProvider" "OIDCRedirectURI" "OIDCClientID" "LicenseFile" "PFC_SegmentedControl_0" "DenyLocal" "DenyLocalExcluded" "Migrate" "MigrateUsersHide" "OIDCROPGID" "OIDCNewPassword" "OIDCAdmin" "OIDCAdminAttribute" "OIDCClientSecret" "OIDCTenant" "OIDCDiscoveryURL" "OIDCIgnoreCookies" "RightsTmpCache" "LoginLogo" "BackgroundImage" "MessageOTPEntry" "HelpURL" "HelpURLLogo" "LocalHelpFile" "EULATitle" "EULASubTitle" "EULAText" "EULAPath" "CreateAdminUser" "EnableFDE" "EnableFDERecoveryKey" "EnableFDERecoveryKeyPath" "ScriptPath" "ScriptArgs" "DemobilizeUsers" ];
...
in
{
  options.programs.macprofile.payloads."managed-applications-com-jamf-connect-login-OpenID" = lib.mkOption {
    type = types.attrsOf (types.submodule payloadModule);
    default = { };
    description = "Jamf Connect Login (OIDC) (com.jamf.connect.login) payload instances, keyed by instance name. Use \"default\" if you only need one.";
  };
}