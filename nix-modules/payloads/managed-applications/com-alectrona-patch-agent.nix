  ...
  payloadModule = {
    options = {
      ...
      _keyNames = lib.mkOption {
        ...
        default = [ "InstallOrUpdate" "UpdateOnly" "OtherManagedSoftware" "RunInterval" "SkipFirstPatchAgentRun" "ActiveIntervals" "RemovePatchSymlink" "ReconAfterInstall" "DeferIfRunning" "UpdateMAS" ];
...
in
{
  options.programs.macprofile.payloads."managed-applications-com-alectrona-patch-agent" = lib.mkOption {
    type = types.attrsOf (types.submodule payloadModule);
    default = { };
    description = "Alectrona Patch Agent (com.alectrona.patch-agent) payload instances, keyed by instance name. Use \"default\" if you only need one.";
  };
}