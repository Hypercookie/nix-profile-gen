  ...
  payloadModule = {
    options = {
      ...
      _keyNames = lib.mkOption {
        ...
        default = [ "labMode" "detailLevel" "labelCase" "dhcpLeaseRemaining" "macAddressFormat" "networkDetailLevel" "subnetMaskCIDRNotation" "coreCount" "diskFreeSpace" ];
...
in
{
  options.programs.macprofile.payloads."managed-applications-net-glencode-Particulars-Widget" = lib.mkOption {
    type = types.attrsOf (types.submodule payloadModule);
    default = { };
    description = "Particulars Today Extension (Widget) (net.glencode.Particulars.Widget) payload instances, keyed by instance name. Use \"default\" if you only need one.";
  };
}