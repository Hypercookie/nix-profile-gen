  ...
  payloadModule = {
    options = {
      ...
      _keyNames = lib.mkOption {
        ...
        default = [ "PFC_SegmentedControl_0" "AcknowledgedDataCollectionPolicy" "ChannelName" "DisableInsiderCheckbox" "EnableCheckForUpdatesButton" "ExtendedLogging" "HowToCheck" "ManifestServer" "SendAllTelemetryEnabled" "StartDaemonOnAppLaunch" "IgnoreUIOpenAfterInstall" "DisableOptInNotification" "UpdateCache" "UpdateCheckInterval" "UpdateCheckFrequency" "UpdateDeadline.DaysBeforeForcedQuit" "UpdateDeadline.FinalCountDown" "UpdateDeadline.StartAutomaticUpdates" "UpdaterOptimization" "OptionalUpdatesDeferralsDays" "OptionalUpdatesDeferrals" "Applications" "UpdateDeadline.ApplicationsForcedUpdateSchedule" ];
      ...
      DisableOptInNotification = lib.mkOption {
        type = types.nullOr (types.bool);
        default = null;
        description = "When Set to TRUE, prevents Opt-In notification/pop-up window from showing even when HowToCheck is set to Automatic.";
      };
...
in
{
  options.programs.macprofile.payloads."managed-applications-com-microsoft-autoupdate2" = lib.mkOption {
    type = types.attrsOf (types.submodule payloadModule);
    default = { };
    description = "Microsoft AutoUpdate (com.microsoft.autoupdate2) payload instances, keyed by instance name. Use \"default\" if you only need one.";
  };
}