...
let
  payloadModule = {
    options = {
      ...
      MSCOfferToQuitBlockingApps = lib.mkOption {
        type = types.nullOr (types.bool);
        default = null;
        description = "If true, if there are any running blocking_applications for pending updates, a sheet will be displayed allowing the user to direct Managed Software Center to attempt to quit the blocking applications.";
      };
      ...
      MSCOfferToUpdateOthers = lib.mkOption {
        type = types.nullOr (types.bool);
        default = null;
        description = "If there is a mix of pending updates, some of which are blocked by open applications, and some that are not, and this preference is true (and so is MSCOfferToQuitBlockingApps), a button will be shown to allow users to proceed with updating the non-blocked updates.";
      };
      ...
      MSCAllowNotificationWindow = lib.mkOption {
        type = types.nullOr (types.bool);
        default = null;
        description = "If true, users can open the Managed Software Center settings window to specify allowed time windows for notifications.";
      };

      MSCAllowedNotificationWindowStart = lib.mkOption {
        type = types.nullOr (types.int);
        default = null;
        description = "The earliest hour that can be selected for allowed notifications.";
      };

      MSCAllowedNotificationWindowEnd = lib.mkOption {
        type = types.nullOr (types.int);
        default = null;
        description = "Only hours earlier than this can be selected (non-inclusive).";
      };
      ...
      DownloadRetries = lib.mkOption {
        type = types.nullOr (types.int);
        default = null;
        description = "Number of times to retry a failed download. Valid values are 0-10. Set to 0 to disable retries.";
      };

      DownloadRetrySleepSeconds = lib.mkOption {
        type = types.nullOr (types.int);
        default = null;
        description = "How many seconds to wait between download retries. Valid values are 1-30.";
      };
...
in
{
  options.programs.macprofile.payloads."managed-applications-ManagedInstalls" = lib.mkOption {
    type = types.attrsOf (types.submodule payloadModule);
    default = { };
    description = "Munki (ManagedInstalls) payload instances, keyed by instance name. Use \"default\" if you only need one.";
  };
}